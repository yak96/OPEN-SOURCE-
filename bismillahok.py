...
▄▄▄▄· ▄▄▄  ▄• ▄▌▄▄▄▄▄▄▄▄ .    ·▄▄▄▄▄▄▄· ©   
▐█ ▀█▪▀▄ █·█▪██▌•██  ▀▄.▀·    ▐▄▄·▐█ ▀█▪.           Author:  Meledak           maaf gw maling Scnya
▐█▀▀█▄▐▀▀▄ █▌▐█▌ ▐█.▪▐▀▀▪▄    ██▪ ▐█▀▀█▄. Wa : 081322544391
██▄▪▐█▐█•█▌▐█▄█▌ ▐█▌·▐█▄▄▌    ██▌.██▄▪▐█. Status: PREMIUM bapak lo
·▀▀▀▀ .▀  ▀ ▀▀▀  ▀▀▀  ▀▀▀     ▀▀▀ ·▀▀▀▀ '''                 
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© Di Sarankan : [green]Cookiedough[white] ©©©'))
		...
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....TUMBAL LU DH JADI DAGING SATE !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	try:
		...
		print('[×] Cookies Burik Ganti Sana')
		time.sleep(5)
		login_lagi334()
	os.system('clear')
	banner()
	...
	cetak(nel('\tSelamat Datang  [yellow]%s[white] Pembawa Dosa Beban orng tua'%(my_name)))
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('├──> Mau Berapa dosa lu / id target lu  ? : '))
	except ValueError:
	...
	for userr in uid:
		try:
			...
			print('├──> Sinyal Loh Kek Kontoll burik gk modal ')
			exit()
	try:
		print('|')
		...
		print(f'{x}')
		print('├──> Sinyal Lo kek Kontol burik gk modal')
		back()
	except (KeyError,IOError):
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r â”€5.py {h}{P}{b}{loop}{P}/{P}{len(id)}{P} {hh}OK:{P}{ok}{P} {kk}CP:{P}{cp}{x}  {P}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
